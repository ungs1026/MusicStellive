...
foreach($matches[1] AS $key => $row) {
	// 이미 업로드된 이미지
	if ( substr($row, 0, 6) == 'noticeboard/upload') {
    $img_array[] = $row;
    continue;
  }
  ...
  file_put_contents('noticeboard/upload/'. $filename, $data);

  $content = str_replace($row, 'noticeboard/upload/'. $filename, $content);
  $img_array[] = 'noticeboard/upload/' . $filename;
}
...
if ($pwd_hash != '') {
  $sql = "UPDATE board SET name=:name, subject=:subject, content=:content, imglist=:imglist, password=:password
    WHERE idx=:idx";
} else {
  $sql = "UPDATE board SET name=:name, subject=:subject, content=:content, imglist=:imglist
    WHERE idx=:idx";
}
